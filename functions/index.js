...
exports.sendLoginNotification = functions.database
  .ref('users/{uid}/profile/lastLogin')
  .onUpdate(async (change, context) => {
    const before = change.before.val();
    const after = change.after.val();
    if (!after || before === after) return null;

    const profileSnap = await DB.ref('users/' + context.params.uid + '/profile').once('value');
    const profile = (profileSnap.val && profileSnap.val()) || {};
    const name = profile.name || 'Unknown';
    const email = profile.email || context.params.uid;
    const region = profile.region || 'Unknown';
    const provider = profile.provider || 'unknown';
    const lastLogin = formatTimestamp(after);

    const to = ADMIN_EMAILS.length ? ADMIN_EMAILS[0] : DEFAULT_ADMIN_EMAIL;

    if (!SENDGRID_KEY) {
      functions.logger.warn('sendLoginNotification: SendGrid key not configured; notification not sent.');
      return null;
    }

    const subject = `ZBH Pan & Plate user login - ${name}`;
    const nameEsc = escapeHtml(name);
    const emailEsc = escapeHtml(email);
    const regionEsc = escapeHtml(region);
    const providerEsc = escapeHtml(provider);
    const lastLoginEsc = escapeHtml(lastLogin);
    ...
    const html = `
      <div style="font-family: Arial, sans-serif; color: #333;">
        ...
        <table style="border-collapse: collapse;">
          ...
          <tr><td style="padding:4px 8px;"><strong>Country/Region:</strong></td><td>${regionEsc}</td></tr>
          <tr><td style="padding:4px 8px;"><strong>Login method:</strong></td><td>${providerEsc}</td></tr>
          <tr><td style="padding:4px 8px;"><strong>Login time:</strong></td><td>${lastLoginEsc}</td></tr>
        </table>
        <p style="color:#888;font-size:12px;">This notification was sent from a secure backend (Firebase Cloud Functions). No passwords or authentication tokens are included.</p>
      </div>
    `;
    ...
    try {
      await sgMail.send(msg);
      functions.logger.info('Login notification sent to', to);
    } catch (err) {
      functions.logger.error('Failed to send login notification:', err);
    }
    return null;
  });
